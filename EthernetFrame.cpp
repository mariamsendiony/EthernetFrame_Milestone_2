...
{   vector <unsigned int> parameters;
    ...
    cout<<"Number_packets_per_frame"<<Number_packets_per_frame<<'\n';
    unsigned int Number_packets_per_second=Number_packets_per_frame*100;
    cout<<"Number_packets_per_second"<<Number_packets_per_second<<'\n';
    unsigned int total_bytes_duration=(Config::LineRate*pow(10,6)/8)*Config::CaptureSizeMs;
    cout<<total_bytes_duration<<'\n';
    float total_time_seconds=(float)Config::CaptureSizeMs/1000;
    ...
    float time_IFG=float(total_time_seconds-(ethernet_time_seconds*totalPackets));
    cout<<time_IFG<<'\n';
    unsigned int IFG_at_end_bits=time_IFG*((Config::LineRate*pow(10,9)));
    cout<<IFG_at_end_bits<<'\n';
    unsigned int IFG_at_end_bytes=ceil((float)IFG_at_end_bits/8);
